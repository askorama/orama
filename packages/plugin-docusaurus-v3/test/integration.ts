...
const sandbox = process.env.KEEP_SANDBOX_DOCUSAURUS
  ...
  : resolve(tmpdir(), `orama-plugin-docusaurus-v3-${Date.now()}`)
...
await test('plugin is able to generate orama DB at build time', async () => {
  ...
  // Pack the plugin and copy it to the sandbox
  await execute(`npm pack --pack-destination ${sandbox}`, './')
  ...
  await writeFile(packageJsonPath, packageJson.replace(/@VERSION@/g, version), 'utf-8')

  // Install dependencies
  const installResult = await execute('npm i', sandbox)
