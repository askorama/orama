...
export default function OramaPluginDocusaurus(
  ...
  return {
        ...
        ...(isDevelopment &&
          !options.cloud && {
            searchData: Object.fromEntries([
              [
                "current",
                readFileSync(indexPath(ctx.generatedFilesDir, "current")),
              ],
            ]),
          }),
        docsInstances,
    ...
    async postBuild({ outDir }) {
      !options.cloud &&
        (await cp(
          indexPath(ctx.generatedFilesDir, "current"),
          indexPath(outDir, "current")
        ));
    },
  };
}
...
function indexPath(outDir: string, version: string) {
  return resolve(
    outDir,
    "orama-search-index-@VERSION@.json.gz".replace("@VERSION@", version)
  );
}
...
async function deployData({
  ...
  if (deployConfig) {
    const endpointConfig = await fetchEndpointConfig(
      baseUrl,
      deployConfig.oramaCloudAPIKey!,
      deployConfig.indexId!
    );
