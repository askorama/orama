...
By default, Orama uses **the English language analyzer**, but we can override this behavior by setting the property `defaultLanguage` at database initialization, and importing a custom stemmer:
...
You can disable stemming by setting `enableStemming: false` while initializing a new Orama instance:
