...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab>```bash copy npm install @orama/plugin-match-highlight ```</Tab>
  <Tab>```bash copy yarn add @orama/plugin-match-highlight ```</Tab>
  <Tab>```bash copy pnpm add @orama/plugin-match-highlight ```</Tab>
</Tabs>
