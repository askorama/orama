...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab>```bash copy npm install @orama/plugin-parsedoc ```</Tab>
  <Tab>```bash copy yarn add @orama/plugin-parsedoc ```</Tab>
  <Tab>```bash copy pnpm add @orama/plugin-parsedoc ```</Tab>
</Tabs>
...
CommonJS imports are still supported, but you'll need to import the plugin from `@orama/plugin-parsedoc/cjs` instead of `@orama/plugin-parsedoc`:

```diff
- const { populateFromGlob } = require("@orama/plugin-parsedoc");
+ const { populateFromGlob } = require("@orama/plugin-parsedoc/cjs");
```
