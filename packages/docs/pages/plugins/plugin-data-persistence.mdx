...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab>```bash copy npm install @orama/plugin-data-persistence ```</Tab>
  <Tab>```bash copy yarn add @orama/plugin-data-persistence ```</Tab>
  <Tab>```bash copy pnpm add @orama/plugin-data-persistence ```</Tab>
</Tabs>
