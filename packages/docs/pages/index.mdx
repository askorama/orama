...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab>```bash copy npm install @orama/orama ```</Tab>
  <Tab>```bash copy yarn add @orama/orama ```</Tab>
  <Tab>```bash copy pnpm add @orama/orama ```</Tab>
</Tabs>
...
CommonJS imports are still supported, but you'll need to import Orama from `@orama/orama/cjs` instead of `@orama/orama`:
