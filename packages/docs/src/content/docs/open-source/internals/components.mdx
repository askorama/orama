...
<Aside type="caution">The `tokenize` function **CANNOT** be async.</Aside>
...
<Aside type="caution">Remember to export both individual functions and the default object (or creation factory function) to allow users to compose different components.</Aside>
...
<Aside>Yes, we know the example below is completely useless and silly. 😁</Aside>
