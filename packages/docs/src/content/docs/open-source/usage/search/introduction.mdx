...
<Aside type="note" title="Orama 3.0 update!">
Since Orama v3.0.0, all the methods are **sync**.

Some plugins like [Plugin Embeddings](/open-source/plugins/plugin-embeddings) may make the methods async due to the nature of the operations. If that's the case, the plugin documentation will specify it.
</Aside>
