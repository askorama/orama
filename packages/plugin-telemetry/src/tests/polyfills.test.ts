...
t.test('sendBeacon function', async (t) => {
  ...
  t.test('should use navigator.sendBeacon if available', async (t) => {
    ...
    t.ok(
      global.navigator.sendBeacon.calledWith(endpoint, body),
      'navigator.sendBeacon was called with correct arguments'
    )
    t.end()
  })
  ...
  t.test('should fall back to fetch if navigator.sendBeacon is not available', async (t) => {
    ...
    // @ts-ignore
    t.ok(
      global.fetch.calledWith(sinon.match(endpoint), sinon.match.has('body', body)),
